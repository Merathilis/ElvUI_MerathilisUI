...
function openRaidLib.commHandler.OnReceiveSafeComm(
	self,
	event,
	prefix,
	text,
	...
	if prefix == CONST_COMM_PREFIX_LOGGED then
		...
		if bIsSafe then
			...
			openRaidLib.commHandler.OnReceiveComm(
				event,
				CONST_COMM_PREFIX,
				data,
				channel,
...
function openRaidLib.commHandler.aceComm.OnReceiveComm(
	event,
	prefix,
	text,
	channel,
	...
	if prefix == CONST_COMM_PREFIX then
		...
		if
			dataTypePrefix ~= CONST_COMM_KEYSTONE_DATA_PREFIX
			and dataTypePrefix ~= CONST_COMM_KEYSTONE_DATAREQUEST_PREFIX
			and dataTypePrefix ~= CONST_COMM_RATING_DATA_PREFIX
			and dataTypePrefix ~= CONST_COMM_RATING_DATAREQUEST_PREFIX
		then
	...
	function(self, event, prefix, text, channel, sender, target, zoneChannelID, localID, name, instanceID)
		if prefix == CONST_COMM_PREFIX_LOGGED then
			...
			elseif chunkNumber and totalChunks and data then
				...
				openRaidLib.DiagnosticError(
					"Logged comm in parts missing information, sender:",
					sender,
					"chunkNumber:",
					chunkNumber,
					"totalChunks:",
...
do
	...
	local newTickerHandle = C_Timer.NewTicker(0.05, function()
		...
		commBurstBufferCount = math.min(
			(serverTime ~= commServerTimeLastThrottleUpdate) and commBurstBufferCount + 1 or commBurstBufferCount,
			CONST_COMM_BURST_BUFFER_COUNT
		)
...
function openRaidLib.Schedules.NewUniqueTimer(time, callback, namespace, scheduleName, ...)
	...
	local bCanRunWithoutGroup = timersCanRunWithoutGroup[namespace]
		and timersCanRunWithoutGroup[namespace][scheduleName]
...
local eventFunctions = {
	...
	["UNIT_PET"] = function(unitId)
		if UnitIsUnit(unitId, "player") then
			openRaidLib.Schedules.NewUniqueTimer(1.1, function()
				openRaidLib.internalCallback.TriggerEvent("playerPetChange")
			end, "mainControl", "petStatus_Schedule")
...
openRaidLib.commHandler.RegisterORComm(CONST_COMM_FULLINFO_PREFIX, function(data, sourceName)
	...
	if openRaidLib.sendRequestedAllInfoCooldown > GetTime() then
		...
		openRaidLib.Schedules.NewUniqueTimer(
			openRaidLib.sendRequestedAllInfoCooldown - GetTime(),
			openRaidLib.commHandler.SendFullData,
			"CommHandler",
			"sendFullData_Schedule"
		)
		return
...
function UnitInfoManager.SetUnitInfo(unitName, unitInfo, specId, talentsString, pvpTalentsTableUnpacked)
	...
	local specId, specName, specDescription, specIcon, role, classFile, classLocName =
		GetSpecializationInfoByID(specId or 0)
...
function UnitInfoManager.AddUnitInfo(unitName, specId, talentsString, pvpTalentsTableUnpacked)
	...
	openRaidLib.publicCallback.TriggerCallback(
		...
		UnitInfoManager.UnitData[unitName],
		UnitInfoManager.GetAllUnitsInfo()
	)
end
...
function UnitInfoManager.UpdateUnitInfo(playerName, specId, talentsString, pvpTalentsTableUnpacked)
	...
	UnitInfoManager.SetUnitInfo(
		playerName,
		unitInfo,
		specId or unitInfo.specId,
		talentsString or unitInfo.talents,
		pvpTalentsTableUnpacked or unitInfo.pvpTalents
	)
	openRaidLib.publicCallback.TriggerCallback(
		"UnitInfoUpdate",
		openRaidLib.GetUnitID(playerName),
		UnitInfoManager.UnitData[playerName],
		UnitInfoManager.GetAllUnitsInfo()
	)
end
...
function UnitInfoManager.OnReceiveUnitFullInfo(data, unitName)
	...
	if not versionNumber or versionNumber < UnitInfoManager.Version then
		openRaidLib.DiagnosticError(
			"UnitInfoManager.OnReceiveUnitFullInfo: invalid version number",
			versionNumber,
			UnitInfoManager.Version
		)
		return
	end
...
function UnitInfoManager.OnPlayerTalentChanged()
	...
	UnitInfoManager.SetUnitInfo(
		...
		unitInfo,
		specId,
		openRaidLib.GetDragonFlightTalentsAsString(),
		UnitInfoManager.GetPlayerPvPTalents()
	)
	...
	openRaidLib.Schedules.NewUniqueTimer(
		2 + math.random(0, 1),
		UnitInfoManager.SendAllPlayerInfo,
		"UnitInfoManager",
		"sendAllPlayerInfo_Schedule"
	)
end
...
function UnitInfoManager.OnLeaveCombat()
	openRaidLib.Schedules.NewUniqueTimer(
		1 + math.random(1, 4),
		UnitInfoManager.SendPlayerInfoAfterCombat,
		"UnitInfoManager",
...
openRaidLib.commHandler.RegisterORComm(
...
function openRaidLib.GearManager.OnReceiveGearFullInfo(data, unitName)
	...
	local equippedGearListUnpacked = equippedGearListIndex
			and openRaidLib.UnpackTable(data, equippedGearListIndex, false, true, 4)
		or {}
...
local checkContext = function()
	...
	if
		...
		or noteContext.difficultyId ~= difficultyID
		or noteContext.instanceType ~= instanceType
	then
		return false
	end
...
function openRaidLib.OpenNotesManager.OnReceiveNoteData(data, unitName)
	...
	if note and type(note) == "string" and commId and type(commId) == "string" then
		...
		openRaidLib.publicCallback.TriggerCallback(
			...
			openRaidLib.GetUnitID(unitName),
			unitNote,
			openRaidLib.OpenNotesManager.GetAllUnitsNotes()
		)
	end
end
openRaidLib.commHandler.RegisterORComm(
	CONST_COMM_OPENNOTES_RECEIVED_PREFIX,
	openRaidLib.OpenNotesManager.OnReceiveNoteData
)
...
openRaidLib.commHandler.RegisterORComm(
	CONST_COMM_OPENNOTES_REQUESTED_PREFIX,
	openRaidLib.OpenNotesManager.OnReceiveNoteRequest
)
...
local cooldownTimeLeftCheck_Ticker = function(tickerObject)
	...
	if timeLeft <= tickerObject.latencyCompensation then
		...
		if
			not openRaidLib.isNearlyEqual(tickerObject.cooldownTimeLeft, timeLeft, CONST_COOLDOWN_TIMELEFT_HAS_CHANGED)
		then
...
function openRaidLib.CooldownManager.OnPlayerTalentChanged()
	...
	openRaidLib.Schedules.NewUniqueTimer(
		...
		openRaidLib.CooldownManager.SendAllPlayerCooldowns,
		"CooldownManager",
		"sendAllPlayerCooldownsFromTalentChange_Schedule"
	)
end
...
local getKeystoneInfoToComm = function()
	...
	local dataToSend = CONST_COMM_KEYSTONE_DATA_PREFIX
		.. ","
		.. keystoneInfo.level
		.. ","
		.. keystoneInfo.mapID
...
function openRaidLib.KeystoneInfoManager.OnReceiveRequestData()
	...
	elseif instanceType == "raid" or instanceType == "pvp" then
		openRaidLib.Schedules.NewUniqueTimer(
			math.random(0, 10) + math.random(1),
			openRaidLib.KeystoneInfoManager.SendPlayerKeystoneInfoToParty,
			"KeystoneInfoManager",
			"sendKeystoneInfoToParty_Schedule"
		)
...
function openRaidLib.WipeRatingData()
	...
	openRaidLib.publicCallback.TriggerCallback(
		...
		unitName,
		ratingInfo,
		openRaidLib.RatingInfoManager.RatingData
	)
	return true
end
...
function openRaidLib.RatingInfoManager.OnReceiveRequestData()
	...
	if instanceType == "party" then
		openRaidLib.Schedules.NewUniqueTimer(
			math.random(1),
			openRaidLib.RatingInfoManager.SendPlayerRatingInfoToParty,
			"RatingInfoManager",
			"sendRatingInfoToParty_Schedule"
		)
	elseif instanceType == "raid" or instanceType == "pvp" then
		openRaidLib.Schedules.NewUniqueTimer(
			math.random(0, 30) + math.random(1),
			openRaidLib.RatingInfoManager.SendPlayerRatingInfoToParty,
			"RatingInfoManager",
			"sendRatingInfoToParty_Schedule"
		)
	...
	if IsInGuild() then
		openRaidLib.Schedules.NewUniqueTimer(
			math.random(0, 6) + math.random(),
			openRaidLib.RatingInfoManager.SendPlayerRatingInfoToGuild,
			"RatingInfoManager",
			"sendRatingInfoToGuild_Schedule"
		)
	end
end
openRaidLib.commHandler.RegisterORComm(
	CONST_COMM_RATING_DATAREQUEST_PREFIX,
					...
					if unitInGroup then
						...
						if spellData then
							...
							if not unitCastCooldown[spellId] or unitCastCooldown[spellId] + 5 < GetTime() then
								...
								openRaidLib.publicCallback.TriggerCallback(
									"CooldownUpdate",
									openRaidLib.GetUnitID(casterName),
									spellId,
									cooldownInfo,
									unitCooldownsTable,
									openRaidLib.CooldownManager.UnitData
