...
local module = MER:GetModule('MER_ChatText')
...
function module:ShortChannel()
	local noBracketsString
	local abbr

	if module.db then
		if module.db.removeBrackets then
			noBracketsString = "|Hchannel:%s|h%s|h"
		end

		if module.db.abbreviation == "SHORT" then
			abbr = abbrStrings[strupper(self)]
		elseif module.db.abbreviation == "NONE" then
			return ""
		else
	...
	if not abbr and module.db.abbreviation == "SHORT" then
		...
		if name then
			...
			if communityID then
				...
				if communityInfo.clubType == 0 then
					if communityInfo.name and module.db and module.db.customAbbreviation then
						abbr = module.db.customAbbreviation[communityInfo.name]
					end
				...
				elseif communityInfo.clubType == 1 then
					if module.db and module.db.customAbbreviation then
						abbr = module.db.customAbbreviation[communityInfo.name]
					end
				...
				if module.db and module.db.customAbbreviation then
					abbr = module.db.customAbbreviation[name]
				end
...
function module:HandleShortChannels(msg)
	msg = gsub(msg, "|Hchannel:(.-)|h%[(.-)%]|h", module.ShortChannel)
	...
	if module.db and module.db.abbreviation == "SHORT" and MER.ChineseLocale then
	...
	if module.db and module.db.removeBrackets then
		if module.db.abbreviation == "SHORT" then
...
function module:ChatFrame_MessageEventHandler(frame, event, arg1, arg2, arg3, arg4, arg5, arg6, arg7, arg8, arg9, arg10, arg11, arg12, arg13, arg14, arg15, arg16, arg17, isHistory, historyTime, historyName, historyBTag)
	...
	if strsub(event, 1, 8) == "CHAT_MSG" then
		...
		if (chatType == "SYSTEM" or chatType == "SKILL" or chatType == "CURRENCY" or chatType == "MONEY" or chatType == "OPENING" or chatType == "TRADESKILLS" or chatType == "PET_INFO" or chatType == "TARGETICONS" or chatType == "BN_WHISPER_PLAYER_OFFLINE") then
		...
		elseif strsub(chatType, 1, 11) == "ACHIEVEMENT" then
			...
			frame:AddMessage(format(arg1, GetPlayerLink(arg2, (noBrackets and "%s" or "[%s]"):format(module:HandleName(coloredName)))), info.r, info.g, info.b, info.id, nil, nil, isHistory, historyTime)
		elseif strsub(chatType, 1, 18) == "GUILD_ACHIEVEMENT" then
			local message = format(arg1, GetPlayerLink(arg2, (noBrackets and "%s" or "[%s]"):format(module:HandleName(coloredName))))
		...
		elseif chatType == "CHANNEL_LIST" then
			if channelLength > 0 then
				frame:AddMessage(format(_G["CHAT_" .. chatType .. "_GET"] .. arg1, tonumber(arg8), arg4), info.r, info.g, info.b,
					info.id,
					nil,
					nil,
					isHistory,
		...
		elseif chatType == "CHANNEL_NOTICE_USER" then
			...
			elseif arg1 == "INVITE" then
				...
				frame:AddMessage(
					format(globalstring, arg8, arg4, arg2), info.r, info.g, info.b, info.id, nil, nil, isHistory, historyTime)
			end
		...
		elseif chatType == "BN_INLINE_TOAST_ALERT" then
			...
			elseif arg1 == "FRIEND_ONLINE" or arg1 == "FRIEND_OFFLINE" then
				...
				if clientProgram and clientProgram ~= "" then
					local name = BNet_GetValidatedCharacterName(characterName, battleTag, clientProgram) or ""
					local characterNameText = BNet_GetClientEmbeddedTexture(clientProgram, 14) .. name
...
function module:ToggleReplacement()
	...
	if self.db.enable and (self.db.abbreviation ~= "DEFAULT" or self.db.removeBrackets) then
		if not initRecord.HandleShortChannels then
			module.cache.HandleShortChannels = CH.HandleShortChannels
			CH.HandleShortChannels = module.HandleShortChannels
			initRecord.HandleShortChannels = true
		end
	else
		if initRecord.HandleShortChannels then
			if module.cache.HandleShortChannels then
				CH.HandleShortChannels = module.cache.HandleShortChannels
			end
