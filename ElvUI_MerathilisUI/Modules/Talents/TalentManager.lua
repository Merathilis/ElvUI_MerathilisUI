...
function module:SaveSet(setName)
	...
	if isSameName then
		MER:Print(format(L["Already have a set named %s."], setName))
	elseif #self.db.sets[self.specID] == 15 then
		MER:Print(L["Too many sets here, please delete one of them and try again."])
	else
...
function module:UpdateSet(setName)
	local talentString = self:GetTalentString()
	local pvpTalentTable = self.db.pvpTalent and self:GetPvPTalentTable()
...
function module:RenameSet(specID, setName, newName)
...
function module:DeleteSet(specID, setName)
	if not self.db.sets[specID] then
		return
	end
...
function module:SetTalent(talentString, pvpTalentTable)
	if talentString and talentString ~= "" then
		local talentTable = {}
		gsub(talentString, "[0-9]",
			function(char)
		...
		if #talentTable < MAX_TALENT_TIERS then
			MER:Print(L["Talent string is not valid."])
		end
...
function module:GetTalentString()
	local talentString = ""
	for tier = 1, MAX_TALENT_TIERS do
		local isAvilable, column = GetTalentTierInfo(tier, 1)
...
function module:GetPvPTalentTable()
...
function module:UpdatePlayerInfo()
...
function module:UpdateSetButtons()
...
function module:ShowContextText(button)
	local menu = {
		...
		},
		{
			text = L["Overwrite"],
			func = function()
				module:UpdateSet(button.setName, true)
			end,
			notCheckable = true
		},
		{
			text = L["Rename"],
			func = function()
				E:StaticPopup_Show("MER_TALENT_MANAGER_RENAME_SET", nil, nil, button)
			end,
			notCheckable = true
		},
		{
			text = L["Delete"],
			func = function()
				module:DeleteSet(button.specID, button.setName)
			end,
			notCheckable = true
		}
	}
...
function module:GetTalentTooltipLine(tier, column)
...
function module:GetPvPTalentTooltipLine(id)
	local lineTemplate = "|T%s:12:14:0:0:32:32:2:30:4:28|t %s"
	if not id then
		return format(lineTemplate, 134400, L["Not set"])
	else
...
function module:BuildFrame()
	...
	local newButton = CreateFrame("Button", "MER_TalentManagerNewButton", frame, "UIPanelButtonTemplate")
	...
	newButton:SetScript(
		...
		function()
			...
			E:StaticPopup_Show("MER_TALENT_MANAGER_NEW_SET", nil, nil, nextIndex)
		end
	)
	S:HandleButton(newButton)

	frame.setButtons = {}
	for i = 1, 15 do
		local button = CreateFrame("Button", "MER_TalentManagerSetButton" .. i, frame, "UIPanelButtonTemplate")
		button:Size(140, 20)
		...
		button:SetScript("OnClick", function(self, mouseButton)
			if mouseButton == "LeftButton" then
				module:SetTalent(self.talentString, self.pvpTalentTable)
	...
	local toggleButton = CreateFrame("Button", "MER_TalentManagerToggleButton", _G.PlayerTalentFrameTalents, "UIPanelButtonTemplate")
	...
	toggleButton:SetScript("OnClick", function()
		if frame:IsShown() then
			module.db.forceHide = true
			frame:Hide()
		else
			module.db.forceHide = nil
			frame:Show()
		end
	end)
...
function module:Enviroment()
	E.PopupDialogs["MER_TALENT_MANAGER_NEW_SET"] = {
	...
	E.PopupDialogs["MER_TALENT_MANAGER_RENAME_SET"] = {
		...
		EditBoxOnEnterPressed = function(self, data)
			...
			if newName then
				module:RenameSet(data.specID, data.setName, newName)
			end
			self:GetParent():Hide()
		end,
		EditBoxOnEscapePressed = function(self)
	...
	self.contextMenuFrame = CreateFrame("Frame", "MER_TalentManagerContextMenu", E.UIParent, "UIDropDownMenuTemplate")
end
