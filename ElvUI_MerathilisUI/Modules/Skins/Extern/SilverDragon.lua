...
local function StyleSilverDragonLootWindow(frame)
	if not frame or frame.__MERStyled then
		return
	end
	...
	if frame.buttons then
		for _, button in pairs(frame.buttons) do
			if not button.IsStyled then
				ES:HandleIcon(button.icon, true)
				ES:HandleIconBorder(button.IconBorder, button.icon.backdrop)
...
local function StyleSilverDragonHistoryLine(line)
	if not line or line.__MERStyled then
		return
	end

	-- Style line as button
	ES:HandleButton(line)
...
local function StyleSilverDragonVisibleHistoryLines(scrollBox)
	...
	if frames then
		for _, line in ipairs(frames) do
			if line and not line.__MERStyled then
...
local function StyleSilverDragonHistoryWindow(frame)
	...
	if frame.collapseButton then
		...
		hooksecurefunc(frame.collapseButton, "SetButtonMode", function(button, mode)
			for _, tex in next, { normalTex, pushedTex, disabledTex } do
				tex:SetTexture(mode == "Plus" and I.Media.Icons.Plus or I.Media.Icons.Minus)
			end
		end)
	end
	...
	if frame.clearButton then
		...
		for _, tex in next, { normalTex, highlightTex, pushedTex, disabledTex } do
			tex:SetTexture(I.Media.Icons.Delete)
			tex:ClearAllPoints()
			tex:SetPoint("CENTER")
			tex:Size(14, 14)
		end
	...
	if frame.container then
		...
		if not container.__MERStyled then
			if container.scrollBox then
				ES:HandleFrame(container.scrollBox)
			end
			if container.scrollBar then
				ES:HandleScrollBar(container.scrollBar)
			end

			container.__MERStyled = true
		end
