		...
		elseif grandChild.ScrollTarget and grandChild.ForEachFrame then
			local function ReskinScrollBox()
				grandChild:ForEachFrame(function(frame)
					...
					module:Proxy("HandleIcon", itemIconComponent.icon)
...
local function ReskinFlyoutFrame(parent)
	for _, child in pairs({ parent:GetChildren() }) do
		if child.layoutType == "PortraitFrameTemplate" and child.TitleContainer then
			if child.__MERSkin then
				return
			end
			module:Proxy("HandleFrame", child)
...
local function ReskinCollectionTab(frame)
	for _, child in pairs({ frame:GetChildren() }) do
		...
		elseif child.obj and child.obj.bar then
			module:Proxy("HandleStatusBar", child.obj.bar)
...
function module:LegionRemixHelper()
	if not E.private.mui.skins.enable or not E.private.mui.skins.addonSkins.legionRemixHelper then
		return
	end
	...
	if C_AddOns_IsAddOnLoaded("Blizzard_ScrappingMachineUI") then
		...
		module:AddCallbackForAddon("Blizzard_ScrappingMachineUI", ReskinScrappingUI)
	end

	if C_AddOns_IsAddOnLoaded("Blizzard_Collections") then
		ReskinCollectionTabUI()
	else
		module:AddCallbackForAddon("Blizzard_Collections", ReskinCollectionTabUI)
	end
end

module:AddCallbackForAddon("LegionRemixHelper")
