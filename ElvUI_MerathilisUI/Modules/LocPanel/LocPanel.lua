...
module.Spells = {
	...
	["challenge"] = {
		...
		[42] = { text = GetSpellInfo(393267), icon = F.GetIconFromID('spell', 393267), secure = { buttonType = 'spell', ID = 393267 }, UseTooltip = true }, -- Teleport: Brackenhide Hollow (Path of the Rotting Woods)
		...
		[46] = { text = GetSpellInfo(410071), icon = F.GetIconFromID('spell', 410071), secure = { buttonType = 'spell', ID = 410071 }, UseTooltip = true }, -- Teleport: Freehold (Path of the Freebooter)
		...
		[50] = { text = GetSpellInfo(424197), icon = F.GetIconFromID('spell', 424197), secure = { buttonType = 'spell', ID = 424197 }, UseTooltip = true }, -- Teleport to Dawn of the Infinite (Path of Twisted Time)
	},
}
...
function module:CreateLocationPanel()
	...
	-- Mover
	E:CreateMover(loc_panel, "MER_LocPanel_Mover", L["Location Panel"], nil, nil, nil, "ALL,SOLO,MERATHILISUI", nil,
		...
		for i = 1, #module.EngineerItems do
			...
			if (not isToy and (F.BagSearch(ID) and IsUsableItem(ID))) or (isToy and (PlayerHasToy(ID) and C_ToyBox_IsToyUsable(ID))) then
				if data.text then
					...
					if cd or (tonumber(cd) and tonumber(cd) > 2) then
						tmp.text = "|cff636363" ..
							tmp.text .. "|r" .. format(module.CDformats[module.db.portals.cdFormat], cd)
						tinsert(module.MainMenu, tmp)
					else
						tinsert(module.MainMenu, data)
					end
...
function module:PopulateDropdown(click)
	...
	if not full_list then
		if not module.ListUpdating then
			F.Print(L
				["Item info is not available. Waiting for it. This can take some time. Menu will be opened automatically when all info becomes available. Calling menu again during the update will cancel it."]); module.ListUpdating = true
		end
	...
	if module.ListUpdating then
		module.ListUpdating = false; F.Print(L["Update complete. Opening menu."])
	end
				...
				tinsert(module.MainMenu,
					{
						...
						func = function()
							...
							MENU_WIDTH = module.db.portals.customWidth and module.db.portals.customWidthValue or
								_G["MER_LocPanel"]:GetWidth()
							tinsert(module.SecondaryMenu,
								{
									text = "<< " .. BACK,
									func = function()
										twipe(module.MainMenu); ToggleFrame(module.Menu2); module:PopulateDropdown()
									end
								})
							tinsert(module.SecondaryMenu,
							...
							tinsert(module.SecondaryMenu,
								{
									...
									ending = true,
									func = function()
										twipe(module.MainMenu); twipe(module.SecondaryMenu); ToggleFrame(module.Menu2)
									end
								})
							DD:DropDown(module.SecondaryMenu, module.Menu2, anchor, point, 0, 1, _G["MER_LocPanel"],
								MENU_WIDTH, module.db.portals.justify)
						end
					})
			end
				...
				tinsert(module.MainMenu,
					{
						...
						func = function()
							...
							tinsert(module.SecondaryMenu,
								{ text = L["Portals"] .. ":", title = true, nohighlight = true })
	...
	tinsert(module.MainMenu,
		{
			...
			ending = true,
			func = function()
				twipe(module.MainMenu); twipe(module.SecondaryMenu); ToggleFrame(module.Menu1)
			end
		})
	MENU_WIDTH = module.db.portals.customWidth and module.db.portals.customWidthValue or _G["MER_LocPanel"]:GetWidth()
	DD:DropDown(module.MainMenu, module.Menu1, anchor, point, 0, 1, _G["MER_LocPanel"], MENU_WIDTH,
