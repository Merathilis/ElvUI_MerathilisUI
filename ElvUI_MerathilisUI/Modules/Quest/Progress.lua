...
local QP = MER:GetModule("MER_QuestProgress") ---@class QuestProgress : AceModule, AceEvent-3.0
local A = MER:GetModule("MER_Announcement")
...
function QP:HandleQuestProgress(status, questData, objectiveData)
	...
	if status == "accepted" then
		...
		coloredContext.icon = format("|T%s:0|t", I.Media.Icons.Accept)
	...
	elseif status == "update" then
		...
		coloredContext.icon = objectiveData.finished and format("|T%s:0|t", I.Media.Icons.Complete) or ""
	end
...
do
	...
	function QP:UpdateBlizzardQuestMessage()
		...
		_G.ERR_QUEST_ADD_ITEM_SII = disabled and ERR_QUEST_ADD_ITEM_SII or MER.UIERRORFRAME_IGNORE_PATTERN
		_G.ERR_QUEST_ADD_FOUND_SII = disabled and ERR_QUEST_ADD_FOUND_SII or MER.UIERRORFRAME_IGNORE_PATTERN
		_G.ERR_QUEST_ADD_KILL_SII = disabled and ERR_QUEST_ADD_KILL_SII or MER.UIERRORFRAME_IGNORE_PATTERN
		_G.ERR_QUEST_UNKNOWN_COMPLETE = disabled and ERR_QUEST_UNKNOWN_COMPLETE or MER.UIERRORFRAME_IGNORE_PATTERN
		_G.ERR_QUEST_OBJECTIVE_COMPLETE_S = disabled and ERR_QUEST_OBJECTIVE_COMPLETE_S
			or MER.UIERRORFRAME_IGNORE_PATTERN
	end
end
...
function QP:Initialize()
	self.db = E.db.mui.quest.progress
