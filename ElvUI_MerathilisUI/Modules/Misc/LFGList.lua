...
function LL:ReskinIcon(parent, icon, role, data)
	...
	if role then
		if self.db.icon.reskin then
			...
			if pack == "SQUARE" then
				icon:SetTexture("INTERFACE\\AddOns\\ElvUI_MerathilisUI\\Media\\Textures\\UI-LFG-ICON-ROLES")
	...
	if parent then
		if self.db.icon.leader then
			if not icon.leader then
				...
				leader:SetTexture(I.Media.Icons.leader)
				leader:Size(10, 8)
...
function LL:InitializePartyKeystoneFrame()
	...
	F.SetFontDB(frame.title, self.db.partyKeystone.font)
	frame.title:Point("TOPLEFT", frame, "TOPLEFT", 8, -10)
	frame.title:SetJustifyH("LEFT")
	frame.title:SetText(F.GetMERStyleText(L["Party Keystone"]))
	...
	F.SetFontDB(frame.detailsButton.Text, self.db.partyKeystone.font)
	...
	F.SetFontDB(frame.sendToPartyChat.Text, self.db.partyKeystone.font)
	...
	for i = 1, 5 do
		...
		F.SetFontDB(rightText, self.db.partyKeystone.font)
		...
		F.SetFontDB(leftText, self.db.partyKeystone.font)
...
function LL:UpdatePartyKeystoneFrame()
	...
	for i = 1, 5 do
		...
		if mapID and MER.MythicPlusMapData[mapID] then
			local mapData = MER.MythicPlusMapData[mapID]
	...
	for i = 1, 5 do
		...
		F.SetFontDB(frame.lines[i].right, self.db.partyKeystone.font)
		...
		F.SetFontDB(frame.lines[i].left, self.db.partyKeystone.font)
...
function LL:InitializeRightPanel()
	...
	frame:SetWidth(MER.ChineseLocale and 190 or 220)
	...
	frame.affix:SetScript("OnEnter", function()
		...
		_G.GameTooltip:ClearLines()
		_G.GameTooltip:AddLine(F.GetMERStyleText(L["Affixes"]))
	...
	local mapIDs = {}
	for key in pairs(MER.MythicPlusMapData) do
		tinsert(mapIDs, key)
	end
	...
	for i, mapID in ipairs(mapIDs) do
		...
		filterButton.name:SetText(MER.MythicPlusMapData[mapID].abbr)
	...
	leaderScore:SetScript("OnEnter", function(btn)
		...
		_G.GameTooltip:AddLine(F.GetMERStyleText(L["Leader Score"]), 1, 1, 1)
	...
	leaderDungeonScore:SetScript("OnEnter", function(btn)
		_G.GameTooltip:SetOwner(btn, "ANCHOR_TOP", 0, 4)
		_G.GameTooltip:AddLine(F.GetMERStyleText(L["Leader's Dungeon Score"]), 1, 1, 1)
	...
	roleAvailable:SetScript("OnEnter", function(btn)
		_G.GameTooltip:SetOwner(btn, "ANCHOR_TOP", 0, 4)
		_G.GameTooltip:AddLine(F.GetMERStyleText(L["Role Available"]), 1, 1, 1)
		_G.GameTooltip:AddLine(
			format(
				"%s %s %s",
	...
	needTank:SetScript("OnEnter", function(btn)
		_G.GameTooltip:SetOwner(btn, "ANCHOR_TOP", 0, 4)
		_G.GameTooltip:AddLine(F.GetMERStyleText(L["Has Tank"]), 1, 1, 1)
		_G.GameTooltip:AddLine(
			format(
				"%s %s",
	...
	needHealer:SetScript("OnEnter", function(btn)
		_G.GameTooltip:SetOwner(btn, "ANCHOR_TOP", 0, 4)
		_G.GameTooltip:AddLine(F.GetMERStyleText(L["Has Healer"]), 1, 1, 1)
		_G.GameTooltip:AddLine(
			format(
				"%s %s",
	...
	vaultStatus:SetScript("OnEnter", function(btn)
		...
		_G.GameTooltip:SetOwner(btn, "ANCHOR_TOP", 0, 4)
		_G.GameTooltip:AddLine(F.GetMERStyleText(L["The Great Vault"]), 1, 1, 1)
		_G.GameTooltip:AddLine(" ")
			...
			if btn.cache[i] then
				...
				_G.GameTooltip:AddDoubleLine(
					format(
						...
						F.GetIconString(MER.MythicPlusMapData[btn.cache[i].mapID].tex, 14, 16, true),
						MER.MythicPlusMapData[btn.cache[i].mapID].name
					),
	...
	sortModeButton.tex:SetTexture(I.Media.Textures.arrow)
	...
	sortByButton.title:SetText(F.GetMERStyleText(L["Sort by"]))
...
function LL:UpdateAdvancedFilters()
	...
	for mapID, data in pairs(MER.MythicPlusMapData) do
		if dfDB[mapID] then
