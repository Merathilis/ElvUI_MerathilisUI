...
local function combat(self, event, unit, eventType, flags, amount, dtype)
	...
	if not E.db.unitframe.units[unit].cft.text.enable then return end
	...
	fontHeight = E.db.unitframe.units[unit].cft.text.size--FeedbackText.origHeight -- always start at original height
	local text, arg
...
local function addCombat(object)
	...
	local set = object.CombatFeedbackText
	local font, fontHeight, fontFlags = set.font, set.fontSize, set.fontStyle--object.CombatFeedbackText:GetFont()
