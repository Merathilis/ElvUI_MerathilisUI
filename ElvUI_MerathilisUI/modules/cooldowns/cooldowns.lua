...
local MERCO = E:NewModule("mUICooldowns", "AceEvent-3.0", "AceHook-3.0")
MERCO.modName = L["Cooldowns"]
...
function MERCO:CreateCooldownIcon(index)
	local button = CreateFrame("Button", "MerathilisUI_CooldownsButton"..index, self.Holder)
...
function MERCO:RemoveCooldown(type, spellID)
...
function MERCO:AddCooldown(type, spellID)
	local n = visible + 1
...
function MERCO:CacheBook(btype)
	local lastID
...
function MERCO:CheckSpellBook(btype)
	...
	for i = 0, (self.db.showbags and 18) or 0, 1 do
		for j = 1, GetContainerNumSlots(i), 1 do
			...
			if enable == 1 then
				if start > 0 then
					if duration > 5 and duration < 3601 then
						MERCO:AddCooldown("inventory", string.join("#", i, j))
					end
				else
					MERCO:RemoveCooldown("slot", i)
				end
...
function MERCO:SPELL_UPDATE_COOLDOWN()
...
function MERCO:PET_BAR_UPDATE_COOLDOWN()
	for i = 1, 10, 1 do
...
function MERCO:PLAYER_ENTERING_WORLD(event)
...
function MERCO:Initialize()
	self.db = E.db.mui.misc.cooldowns
	if not self.db.enable then return end

	self.Holder = CreateFrame("Frame", nil, E.UIParent)
	self.Holder:Size(self.db.size)
	self.Holder:Point("BOTTOMRIGHT", E.UIParent, "BOTTOM", -80, 530)
	E:CreateMover(self.Holder, "Cooldown Mover", L["Cooldown Mover"], true, nil, "ALL,GENERAL,ACTIONBARS")

	self:RegisterEvent("PLAYER_ENTERING_WORLD")
end

E:RegisterModule(MERCO:GetName())