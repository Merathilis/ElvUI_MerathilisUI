...
LP.Spells = {
	...
	["teleports"] = {
		["Horde"] = {
			...
			[12] = {text = GetSpellInfo(224873),icon = MER:GetIconFromID("spell", 224873),secure = {buttonType = "spell",ID = 224873}, UseTooltip = true}, -- TP:Dalaran - BI
...
function LP:ItemList(check)
	for i = 1, #LP.PortItems do
		...
		if MER:BagSearch(data.secure.ID) or (PlayerHasToy(data.secure.ID) and IsToyUsable(data.secure.ID)) then
			if check then 
				if LP.db.portals.HSplace then tinsert(LP.MainMenu, {text = HOME..": "..GetBindLocation(), title = true, nohighlight = true}) end
