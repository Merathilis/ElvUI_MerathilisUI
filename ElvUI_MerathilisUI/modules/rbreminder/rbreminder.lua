...
function RB:CreateIconBuff(name, relativeTo, firstbutton)
	...
	button.backdrop:SetPoint("TOPLEFT", E:Scale(-2), E:Scale(-2))
	button.backdrop:SetPoint("BOTTOMRIGHT", E:Scale(-2), E:Scale(-2))
...
function RB:Initialize()
	...
	if E.db.mui.raidBuffs.class == true then
		self.frame:Size(bsize*3+98, bsize + 8) -- Background size (needs some adjustments)
